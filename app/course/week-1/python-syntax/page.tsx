...
/**
 * Week 1 • Lesson 1: Python Syntax (Beginner‑first, no MDX)
 * - Mirrors the wiring from the Week 1 Start page
 * - Pure Tailwind UI (no external UI kit)
 * - Progress via `tracking` (user_id, key)
 * - Sticky, responsive sidebar + scrollspy
 * - Friendly tone, analogies, step‑by‑step breakdowns
 * - 🚨 Common Mistake Prevention (pitfalls, warnings, pro tips)
 * - 🏃‍♂️ Interactive runner (Web Worker + Pyodide) with quick‑loads
 */
    ...
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-6">
        ...
        <main className="space-y-8">
          {/* Intro */}
          <section id="intro" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-4">
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-white">Python Syntax, calmly explained</h1>
            <p className="text-gray-700 dark:text-gray-300">Think of Python as talking to the computer in friendly, precise sentences. In this lesson, we’ll build your core vocabulary and grammar one brick at a time.</p>
          ...
          {/* Variables */}
          <section id="variables" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
            <h2 className="text-xl font-semibold">Variables: labeled containers</h2>
            <p className="text-gray-700 dark:text-gray-300">A <em>variable</em> is a label you stick on a value so you can find it later, like a jar on a shelf: <code className="px-1 rounded bg-gray-100 dark:bg-gray-800">sugar</code> → <code className="px-1 rounded bg-gray-100 dark:bg-gray-800">'sweet'</code>.</p>
            <pre className="text-sm bg-gray-50 dark:bg-gray-900 p-3 rounded whitespace-pre-wrap">{`total_cookies = 12
friend_name = 'Ada'
pi = 3.14159
is_hungry = True`}</pre>
          ...
          {/* Core types */}
          <section id="types" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
...
print(apples * price)`}</pre>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Use <code>//</code> for floor division and <code>**</code> for powers.</p>
              </div>
              <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-4 bg-gray-50 dark:bg-gray-900">
...
print(greeting.upper(), bool(1), bool(''))`}</pre>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2"><em>Truthiness</em>: empty things are <code>False</code> (<code>''</code>, <code>[]</code>, <code>{'{}'}</code>), others are <code>True</code>.</p>
              </div>
            </div>
          ...
          {/* Printing */}
          <section id="print" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
            <h2 className="text-xl font-semibold">Printing & f‑strings</h2>
            <p className="text-gray-700 dark:text-gray-300">Use <code>print()</code> to see what’s going on. f‑strings are like fill‑in‑the‑blank templates.</p>
            <pre className="text-sm bg-gray-50 dark:bg-gray-900 p-3 rounded whitespace-pre-wrap">{`name = 'Grace'
score = 93.756
print(f'Hello {name}, score: {score:.1f}')`}</pre>
          ...
          {/* Control flow */}
          <section id="control" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
            <h2 className="text-xl font-semibold">Decisions: <code>if</code> / <code>elif</code> / <code>else</code></h2>
            <p className="text-gray-700 dark:text-gray-300">Tell the computer what to do depending on a condition.</p>
            <pre className="text-sm bg-gray-50 dark:bg-gray-900 p-3 rounded whitespace-pre-wrap">{`temp = 28
if temp >= 30:
    status = 'hot'
elif temp >= 20:
    status = 'warm'
...
print(status)`}</pre>
          ...
          {/* Loops */}
          <section id="loops" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
...
while i < len(nums):
    ...
    i += 1
print(total)`}</pre>
                <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">Use <code>while</code> when you don’t know how many times you’ll loop.</p>
              </div>
            </div>
          ...
          {/* Helpers */}
          <section id="helpers" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
            <h2 className="text-xl font-semibold">Handy helpers: <code>range</code>, <code>enumerate</code>, <code>zip</code></h2>
            <pre className="text-sm bg-gray-50 dark:bg-gray-900 p-3 rounded whitespace-pre-wrap">{`for i in range(3):
    print('i =', i)
...
for x, y in zip(xs, ys):
    print(x, y)`}</pre>
          ...
          {/* Functions */}
          <section id="functions" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
            <h2 className="text-xl font-semibold">Functions: your own commands</h2>
            <p className="text-gray-700 dark:text-gray-300">A function is a recipe: inputs (ingredients) go in, steps run, and a result comes out.</p>
            <pre className="text-sm bg-gray-50 dark:bg-gray-900 p-3 rounded whitespace-pre-wrap">{`def greet(name='World'):
...
print(greet('Ada'))`}</pre>
          ...
          {/* Pitfalls */}
          <section id="pitfalls" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-4">
...
print(grade_average([88, 92, 79, 90]))`}</pre>
          ...
          {/* Runner */}
          <section id="runner" className="rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-950 p-6 shadow-sm space-y-3">
          ...
          <section className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
            <Link href="/course/week-1" className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
